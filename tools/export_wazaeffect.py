...
class Cmd2dParam():
    ...
    def parse(self, fin):
		...
		#fin.seek(self.address)
		self.byte = fin.readUInt32()
		print("self.byte: " + str(self.byte))
		i = 0
		while i < self.byte:
			self.bytes += [fin.readUInt32()]
			i += 1
		self.size += 4*len(self.bytes)

    def get_dependencies(self, recompute=False, global_dependencies=set()):
        return []

    def to_asm(self):
        output = hex(self.byte)
        for n in self.bytes:
            output += ", " + hex(n)
        return output
	...
	def parse(self, fin):
		...
		while not done:
			...
			for key, param in class_.param_types.items():
				...
				elif (param['class'] == Cmd2dParam):
					print("Cmd2dParam.size: " + hex(class_.params[key].size))
